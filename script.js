...
(() => {
  ...
  /* ------------------------------------------------------------------ *
   *  Scene 3 — typewriter effect for the letter lines
   *  Types each .type-line's data-text out one character at a time,
   *  moving to the next line only once the current one finishes, then
   *  reveals the final (static) line with a soft fade-in.
   * ------------------------------------------------------------------ */
  ...
  async function runTypewriter() {
    ...
    const finalLine = document.querySelector('.letter-line-final');

    for (const line of lines) {
      // eslint-disable-next-line no-await-in-loop
      await typeLine(line, 28);
      // a short beat between lines feels more like handwriting/typing
      // eslint-disable-next-line no-await-in-loop
